...
void loop() {
  ...
  if (peticion.indexOf("gpio2=0") != -1) botonPulsado = 0;
  else if (peticion.indexOf("gpio2=1") != -1) botonPulsado = 1;
  String estadoLed = botonPulsado?"ENCENDIDO":"APAGADO";

  //Enciende/apaga el led
  digitalWrite(ledPin, botonPulsado);
 ...
 //Devuelve una página web como respuesta
  ...
  clienteWeb.println("<center><p>El led situado en GPIO2 está " + estadoLed + "</p></center>");
  clienteWeb.println("<center><button onClick=location.href='./?gpio2=0'>OFF</button>");
  clienteWeb.println("<button onClick=location.href='./?gpio2=1'>ON</button></center>");
  clienteWeb.println("</body>");
  clienteWeb.println("</html>");
